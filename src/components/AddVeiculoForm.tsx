...
export default function AddVeiculoForm({ onClose, onSuccess }: Props) {
  const [form, setForm] = useState({
    ...
    trunk_capacity: '',
    stock: false,
    image_url: '', // <— novo campo de texto com o URL da imagem
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validação simples do URL (opcional)
      ...
      const vehicleData = {
        ...
        speed_tuned: Number(form.speed_tuned),
        trunk_capacity: Number(form.trunk_capacity),
        stock: form.stock,
        image_url: url,          // <— guarda diretamente o URL
        image_public_id: '',     // <— opcional: remove se não usares este campo na coleção
      }
      ...
      console.error('Erro ao adicionar veículo no Firestore:', err)
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col">
              <label htmlFor="brand" className="text-sm font-semibold mb-1 text-gray-700">Marca</label>
              <input
                id="brand"
                type="text"
                name="brand"
            ...
            <div className="flex flex-col">
              <label htmlFor="model" className="text-sm font-semibold mb-1 text-gray-700">Modelo</label>
              <input
                id="model"
                type="text"
                name="model"
            ...
            <div className="flex flex-col">
              <label htmlFor="category" className="text-sm font-semibold mb-1 text-gray-700">Categoria</label>
              <select
                id="category"
                name="category"
                value={form.category}
                ...
                required
              >
                <option value="" disabled hidden>Seleciona uma categoria</option>
            ...
            {['price', 'speed_original', 'speed_tuned', 'trunk_capacity'].map((field) => (
              <div key={field} className="flex flex-col">
                <label htmlFor={field} className="text-sm font-semibold mb-1 text-gray-700">
                  ...
                  field === 'speed_tuned' ? 'Velocidade Tunada (km/h)' :
            ...
            {/* Campo de texto para URL da imagem (substitui o upload) */}
            <div className="md:col-span-2 flex flex-col gap-2">
              ...
              <input
                ...
                className="border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
              />
              {/* Pré-visualização simples (opcional) */}
              {form.image_url && /^https?:\/\/.+/i.test(form.image_url.trim()) && (
                <div className="mt-2">
                  <img
